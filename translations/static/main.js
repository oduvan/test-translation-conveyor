...
function fatch_patch(url, data, all_good) {
    fetch(url, {
        'method': 'PATCH',
        headers: {
...
function action__change_status(obj, data){
    fatch_patch('/translations/' + obj.id + '/', {'from_status': obj.status, 'status': data.status}, ()=>show_table_for(obj.status));
}
...
function send_translation() {
    fatch_patch('/translations/' + current_obj.id + '/', {
